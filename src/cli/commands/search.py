...
def _search(
    ...
    Args:
        ...
        query_transform: 查询变换模式 (none/hyde/multi_query/sub_query/combined)
        context_compress: 是否启用上下文压缩
    """
    # 健康检查（除非明确跳过）
...
def semantic(
    ...
    用法: ath search semantic <项目> <查询>
    示例: ath search semantic yunxi "采矿计划编制"
    """
...
def keyword(
    ...
    用法: ath search keyword <项目> <查询>
    示例: ath search keyword yunxi "业务场景"
    """
    project_id = _resolve_project_id(project)
    if project_id != project:
        console.print(f"[dim]项目: {project} → {project_id[:8]}...[/dim]")
...
def hybrid(
    ...
    query_transform: str = typer.Option(
        "none", "--qt", help="查询变换模式: none/hyde/multi_query/sub_query/combined"
    ),
    compress: bool = typer.Option(False, "--compress", "-c", help="启用上下文压缩"),
    skip_health_check: bool = typer.Option(False, "--skip-health-check", help="跳过健康检查（调试用）"),
):
    """混合搜索 - 向量 + BM25 + RRF 融合（推荐）
    
    用法: ath search hybrid <项目> <查询>
    示例: ath search hybrid yunxi "数据中台架构" -k 5
          ath search hybrid yunxi "数据中台架构" -k 1 --full
          ath search hybrid yunxi "数据中台架构" --qt multi_query
          ath search hybrid yunxi "数据中台架构" --compress
          ath search hybrid yunxi "数据中台架构" --skip-health-check（调试模式）
    """
    project_id = _resolve_project_id(project)
    if project_id != project:
        console.print(f"[dim]项目: {project} → {project_id[:8]}...[/dim]")
    _search(project_id, query, "hybrid", top_k, score_threshold, full, skip_health_check, query_transform, compress)
...
def hierarchical(
    ...
    用法: ath search hierarchical <项目> <查询>
    示例: ath search hierarchical yunxi "项目总体设计"
