...
本次对 `~/Projects/ATHRag/` 项目进行了可靠性加固，主要包括以下四个方面的改进：
...
2. **友好的超时错误信息**
   ...
   - 提供提示：使用 `ath service status` 查看服务状态
...
# 跳过健康检查（调试用）
ath search hybrid yunxi "数据中台架构" --skip-health-check
```
...
# 添加到 cron（每 5 分钟检查一次）
*/5 * * * * /Users/jk/Projects/ATHRag/scripts/health_monitor.sh >> /Users/jk/Projects/ATHRag/logs/health_monitor.log 2>&1
```
...
```bash
# 每天凌晨 3 点执行一致性检查和修复
0 3 * * * /Users/jk/Projects/ATHRag/.venv/bin/python /Users/jk/Projects/ATHRag/scripts/sync_missing_vectors.py >> /Users/jk/Projects/ATHRag/logs/vector_sync.log 2>&1
```
